...
export class CommonService {
  ...
  doGet(apiUrl: String): Observable<ApiResponse> {
    ...
    const loginData = JSON.parse(this.Decrypt(localStorage.getItem(StorageKey.loginData)));
    if (loginData) {
    ...
    return this.http.get<ApiResponse>(url, httpOptions).pipe(
      ...
      catchError(this.handleError<ApiResponse>(
        `doGet url = ${JSON.stringify(apiUrl)}`,
        { Data: null, Message: "Something went wrong. Please try again after sometime.", Success: false, TAID: null })
      )
    );
  }
  ...
  public Encrypt(clearText: string): string {
    var encrypted = CryptoJS.AES.encrypt(CryptoJS.enc.Utf8.parse(clearText), this.key,
      {
        keySize: 128 / 8,
        iv: this.iv,
  ...
  async getMenuListByRoleId(RoleId : any) {
    ...
    let roleRights = await this.doGet(apiUrl).toPromise()
    this.role_rights = roleRights.Data;
  }

  public checkRoleRights(_url: string, type: string) {
    let data = this.role_rights.find(x => x.menuUrl == _url && x.menuUrl != "");
    if (type == 'add') {
