...
import { ActionEvent, ApiConfigModel, ApiResponse, ButtonsConfig, PaginationModel, TableColumn } from 'src/app/core/model/common-model';
...
export class DataListComponent implements AfterViewInit {
  ...
  initList() {
    this.displayedColumns = this.columns.filter(a => a.isShow == true).map(d => d.columnDef);
    ...
    this.fetchRecords().subscribe({
      next: (data) => {
        if (data && data.Success) {
          ...
          this.totalRecord = this.itemList && this.itemList.length > 0 && this.tableType == TableType.Pagination ? this.itemList[0].TotalFilteredRecord : this.itemList?.length;
          this.setTablePagination();
        }
        else {
  ...
  fetchRecords(): Observable<any> {
    ...
    if (this.apiDetails.type == ApiType.Get) {
      apiUrl = this.commonService.getApiURL(apiUrl, this.apiDetails.requestData);
      ...
      return this.commonService.doPost(apiUrl, this.apiDetails.requestData).pipe(
        map((data: ApiResponse) => {
          return data;
        }),
        catchError(error => {
